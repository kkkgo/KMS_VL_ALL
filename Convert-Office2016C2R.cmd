...
for /f "tokens=2*" %%G in ('"reg query HKLM\SOFTWARE\Microsoft\Office\ClickToRun /v InstallPath" 2^>nul') do (
    set "_OSPP=%%H\Office16\OSPP.VBS"&set "_LicPath=%%H\root\Licenses16"
)
...
if %errorlevel% EQU 1060 (
    echo.
    echo Could not detect Office ClickToRun service...
    goto :end
)
...
for /f "tokens=2 delims==" %%G in ('"wmic path %sps% get version /format:list" 2^>nul') do set ver=%%G

wmic path %spp% where (Description like '%%KMSCLIENT%%' AND LicenseFamily != 'Office16MondoR_KMS_Automation') get Name /format:list 2>nul | findstr /i /C:"Office 16" 1>nul && set /a _VL=1

wmic path %spp% where (Description like '%%TIMEBASED%%') get Name /format:list 2>nul | findstr /i /C:"Office 16" 1>nul && set /a _TB=1

wmic path %spp% where (Description like '%%Grace%%') get Name /format:list 2>nul | findstr /i /C:"Office 16" 1>nul && set /a _Grace=1
...
echo.
echo Cleaning Current Office 16 Licenses...
cd /d "%~dp0"
...
:Ret2VL
echo.
echo Installing Office 16 Volume Licenses...
cd /d "%_LicPath%"
...
for /d %%G in %SkuIds% do (set /a _%%G=0)
for /d %%G in %ProSkuIds% do (set /a _%%G=0)
for /d %%G in %ProSkuId2% do (set /a _%%G=0)

for /f "tokens=2,*" %%G in ('"reg query HKLM\SOFTWARE\Microsoft\Office\ClickToRun\Configuration /v ProductReleaseIds" 2^>nul') do (set "ProductIds=%%H")

for /d %%G in %SkuIds% do (
    echo %ProductIds% | findstr /I /C:"%%Gretail" 1>nul && set /a _%%G=1
)
...
if %_mondo% EQU 1 (call :InsLic mondo &goto :GVLK)

for /d %%G in %SkuIds% do (
    if !_%%G! EQU 1 call :InsLic %%G
)
...
echo.
for /f "tokens=2 delims==" %%G in ('"wmic path %spp% where (Description like '%%Office 16, VOLUME_KMSCLIENT%%') get ID /format:list"') do (
    set ActID=%%G&call :InsKey
)
goto :end
::========================================================================================================================================================
:InsLic
...
::========================================================================================================================================================
:InsKey
for /f "tokens=2 delims==" %%G in ('"wmic path %spp% where ID='%ActID%' get Name /format:list"') do echo %%G
(echo edition = "%ActID%"
